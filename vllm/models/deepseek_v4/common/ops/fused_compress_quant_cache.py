...
def dsv4_dcp_finalize_sparse_attn_kernel(
    ...
    # Manual e4m3fn encode: bit-equivalent to the hw cast (RNE, saturating)
    # and works on SM8x where Triton cannot emit fp8e4nv.
    x_uint8 = tl.reshape(
        _fp32_to_fp8_e4m3fn_byte(x_clamped),
        (TRITON_BLOCK_SIZE,),
    )
...
def dsv4_dcp_finalize_indexer_attn_kernel(
    ...
    x_clamped = tl.clamp(x_scaled, -FP8_MAX, FP8_MAX)
    # Manual e4m3fn encode (SM8x-safe, bit-equivalent to the hw cast).
