...
class MiMoV2Attention(nn.Module):
    def __init__(
        ...
        # A100/3090 (sm<90) patch: FA3/4 DiffKV is Hopper-only, and the
        # TRITON_ATTN_DIFFKV backend computes MiMo's asymmetric-V + SWA sinks
        # incorrectly. Route sm<90 through the standard TritonAttentionBackend
        # with V padded v_head_dim->head_dim (exact: zero-padded dims >=128
        # are sliced off the output). Hopper keeps the native FA-DiffKV path.
        # Users can force a diff-kv choice via
    ...
    def forward(
        ...
        if self._pad_v:
            # Pad V to head_dim so K/V share a head size for Triton, then
            # slice the zero-padded tail off the output (exact).
